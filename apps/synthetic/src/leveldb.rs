...
#[derive(Clone, Copy)]
pub struct LevelDBProtocol {
    nvalues: u64,
    pct_scan: u64,
}

impl LoadgenProtocol for LevelDBProtocol {
    ...
    fn read_response(&self, mut sock: &Connection, scratch: &mut [u8]) -> io::Result<(usize, u64)> {
        ...
        let header = LevelDBHeader::deserialize(&mut &scratch[..])?;
...
impl LevelDBProtocol {
    pub fn with_args(matches: &clap::ArgMatches, tport: Transport) -> Self {
        ...
        LevelDBProtocol {
            nvalues: 15000, // value_t!(matches, "r_nvalues", u64).unwrap(),
    ...
    pub fn args<'a, 'b>() -> Vec<clap::Arg<'a, 'b>> {
        ...
        .takes_value(true)
        .default_value("5000")
        .help("LevelDB: number of key value pairs"),
            Arg::with_name("pctscan")
		.long("pctscan")
		.takes_value(true)
		.default_value("0")
		.help("LevelDB: scan requests per 1000 requests"),
	]
    }
}
...
impl LevelDBHeader {
    ...
    pub fn deserialize<R: io::Read>(reader: &mut R) -> io::Result<LevelDBHeader> {
        let p = LevelDBHeader {
