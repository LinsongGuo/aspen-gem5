...
#endif
{
    ...
    // BASKET basket[B+K+1];
    // BASKET *perm[B+K+1];
    BASKET *basket = (BASKET*) malloc(sizeof(BASKET) * (B+K+1));
    // printf("basket: %ld, %d\n", sizeof(BASKET), (B+K+1));
    ...
    while( !opt )
    {       
        if( (bea = primal_bea_mpp( m, arcs, stop_arcs, &red_cost_of_bea, &nr_group, &group_pos, &initialize, &basket_size, basket, perm)) )
        {
            (*iterations)++;
