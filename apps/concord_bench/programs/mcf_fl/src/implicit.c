...
#endif
{
    ...
    for( node++, stop = (void *)net->stop_nodes; node < stop; node++ ) // count: 23944
        if( node->pred != root )
...
#endif
{
    ...
    while( pos-1 && red_cost > (cost_t)new[pos/2-1].flow ) // count: 3501808
    {
...
#endif
{
    ...
    while( cmp <= net->max_residual_new_m && red_cost < new[cmp-1].flow ) // count: 0
    {
...
#endif
{
    ...
    arcout = net->arcs;
    // #pragma clang loop unroll_count(1) 
    for( i = 0; i < trips && arcout[1].ident == FIXED; i++, arcout += 3 ); // count: 0
    ...
    if( new_arcs )
    {
        ...
        if( resized )
        {
            ...
            for( ; arcnew != stop; arcnew++ )  // two loops count: 2393292
            {
        ...
        {
            // #pragma clang loop unroll_count(1)
            for( ; arcnew != stop; arcnew++ ) // count: few
            {
                arcnew->flow = (flow_t)0;
                arcnew->ident = AT_LOWER;
...
#endif
{
    ...
    else
    {
        ...
        for( susp = 0, arc = new_arc; arc < (arc_t *)stop; arc++ ) // count: 2393292
        {
