...
#endif
{
    ...
    for( stop = (void *)net->stop_nodes; node < (node_t *)stop; node++ )  // count: few
    {
    ...
    arc = net->arcs;
    // #pragma clang loop unroll_count(1)
    for( stop = (void *)net->stop_arcs; arc < (arc_t *)stop; arc++ ) // count: few
    {
...
#endif
{
    ...
    tmp = node = root->child;
    // #pragma clang loop unroll_count(1)
    while( node != root ) // count: 0
    {
        #pragma clang loop unroll_count(4)
        while( node ) // count: 12390246 
        {
...
#endif
{
    ...
    // #pragma clang loop unroll_count(1) // count: 0
...
#endif
{
    ...
    stop = (void *)net->stop_arcs;
    // #pragma clang loop unroll_count(1) 
    for( arc = net->arcs; arc != (arc_t *)stop; arc++ ) // count: 0 
    {
        if( arc->ident == AT_UPPER )
            arc->flow = (flow_t)1;
        else
    ...
    stop = (void *)net->stop_nodes;
    // #pragma clang loop unroll_count(1) 
    for( node = net->nodes, node++; node != (node_t *)stop; node++ ) // count: 0
        node->basic_arc->flow = node->flow;
...
#endif
{
    ...
    for( node++; node < (node_t *)stop; node++ ) // count: 23940
    {
        arc = node->basic_arc;
        flow = node->flow;
...
#endif
{
    ...
    // #pragma clang loop unroll_count(2)
    for( arc = net->arcs; arc < stop; arc++ ) // count: 0
    {
