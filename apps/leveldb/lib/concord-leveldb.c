...
int cncrd_leveldb_scan_get(leveldb_t *db, const leveldb_readoptions_t *roptions, const char *search, char *key, size_t key_len) {
    PRE_PROTECTCALL;
    leveldb_iterator_t *iter = leveldb_create_iterator(db, roptions);
    POST_PROTECTCALL;
    ...
    while (1) {
        if (i % 1000 == 0) {    
            size_t val_len;
            char *err = NULL;
            char* val = cncrd_leveldb_get(db, roptions, key, key_len, &val_len, &err);
            if (val == NULL || strncmp(key, val, val_len)) {
...
int cncrd_leveldb_scan_iterator(leveldb_t *db, leveldb_iterator_t *iter, const char *search) {
    PRE_PROTECTCALL;
    leveldb_iter_seek_to_first(iter);
    POST_PROTECTCALL;
