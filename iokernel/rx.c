...
static struct rx_net_hdr *rx_prepend_rx_preamble(struct rte_mbuf *buf)
{
	...
	masked_ol_flags = buf->ol_flags & PKT_RX_IP_CKSUM_MASK; // RTE_MBUF_F_RX_IP_CKSUM_MASK;
	if (masked_ol_flags == PKT_RX_IP_CKSUM_GOOD) // RTE_MBUF_F_RX_IP_CKSUM_GOOD)
...
static void rx_one_pkt(struct rte_mbuf *buf)
{
	...
	ptr_dst_addr = &ptr_mac_hdr->d_addr; // ptr_dst_addr = &ptr_mac_hdr->dst_addr;
